...
\begin{itemize}
    ...
    \item Performance prediction methods (also one-shot techniques)
    \begin{itemize}
        \item Zero-cost proxies
    \end{itemize}
    \item Low-fidelity estimates
    \end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item SInce then, goal is to speed up the evaluation
\end{itemize}
\end{frame}

\begin{frame}{Performance Prediction}
\begin{amsdefi}[\textbf{Performance Prediction}]
Given a search space $\mathcal{A}$, an architecture $a \in \mathcal{A}$ and the final validation accuracy $f(a)$.
A \textit{performance predictor} $f'(a)$ is the function, which predicts the score of the architecture $a$, without the full training pipeline $P$. $f'(a)$ needs less time for evaluations than $f(a)$. Also $\{f'(a) \vert a \in A\}$ is ideally highly correlated with  $\{f(a) \vert a \in A\}$.
\end{amsdefi}
\end{frame}
...
\item Can be wrapped in different disguises
\end{enumerate}
\end{itemize}
...
% Text Node
\draw (484.38,167.6) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize,color={rgb, 255:red, 226; green, 0; blue, 122 }  ,opacity=1 ] [align=left] {$\displaystyle f( a)$};
% Text Node
\draw (85.48,177.98) node  [font=\footnotesize] [align=left] {\begin{minipage}[lt]{84.22pt}\setlength\topsep{0pt}
\begin{center}
Data:\\ $\displaystyle ( a\ \in \ \mathcal{A} ,\ f( a))_{i=1,...,N}$
\end{center}
...
\begin{itemize}
    ...
    \end{itemize}
    \item Prediction method for the target $f(a)$
...
% Text Node
\draw (496.38,69.6) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize,color={rgb, 255:red, 226; green, 0; blue, 122 }  ,opacity=1 ] [align=left] {$\displaystyle f( a)$};
% Text Node
...
\begin{minipage}{0.1\textwidth}
    \begin{center}
		\includegraphics[width=\textwidth, height=2.5cm]{07_NAS/images/04/example_graph_encoding.pdf}
        \subcaption{DAG of a neural network cell}
    \end{center}
\end{minipage}
\hspace{1cm}
\begin{minipage}{0.2\textwidth}
	\begin{center}
        \includegraphics[width=\textwidth, height=2.5cm]{07_NAS/images/04/example_adj_encoding.pdf}
        \subcaption{Adjacency matrix~\liturl{Yang et al. 2019}{https://arxiv.org/pdf/1902.09635}}
    \end{center}
\end{minipage}
\hspace{1cm}
\begin{minipage}{0.1\textwidth}
    \begin{center}
	   \includegraphics[width=\textwidth, height=2.5cm]{07_NAS/images/04/example_path_encodig.pdf}
      \subcaption{Path-based encoding~\liturl{White et al. 2021}{https://cdn.aaai.org/ojs/17233/17233-13-20727-1-2-20210518.pdf}}
    \end{center}
\end{minipage}
\end{figure}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}{Zero-Cost Proxies}
    \begin{itemize}
        \item Very recent developed as part of performance prediction 
...
\begin{itemize}
    \item Use performance prediction methods to improve on the search efficiency
    \onslide<2-3>
...
\end{itemize}
    ...
    \let\thefootnote\relax\footnotetext{\vspace{-0.5cm}  \hspace{-0.45cm}
\scriptsize{
{\color{UMblue!70!white}{White et al. \textit{How Powerful are Performance Predictors in Neural Architecture Search?} NeurIPS, 2021.}}}}
