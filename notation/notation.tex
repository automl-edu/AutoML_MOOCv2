...
\begin{table}[H]
	\begin{tabular}{lll}
     ...
     \verb!\(\xI{i},\yI{i})! &	$(\xI{i},\yI{i})$ & $i$-th observation\\
		...
		\midrule
		\multicolumn{3}{c}{\textcolor{red}{\textbf{Ensembling}}}\\
		\verb!\Xspace! & $\Xspace$ & Input space \\
		\verb!\Yspace! & $\Yspace$ & Output space \\
		\verb!\bl[m]! & $\bl[m]$ & $m$-th base model \\
		\verb!\blp[m]! & $\blp[m]$ & Probability prediction of $m$-th base model \\
		\verb!\bll[m]! & $\bll[m]$ & Hard-label prediction of $m$-th base model \\
		\verb!\blx[m]! & $\blx[m]$ & Prediction of $m$-th base model at $\x$ \\
		\verb!\ensweights! & $\ensweights$ & Ensemble weight vector \\
		\verb!\fM! & $\fM$ & Ensemble predictor \\
		...
		\multicolumn{3}{c}{\textcolor{red}{\textbf{Hyperparameter Optimization}}}\\
		 ...
		 \verb!\pcsdim!  & $\pcsdim$ &  Dimensionality of $\pcs$\\
		...
		\verb!\noisevar!  & $\noisevar$ & GP observation noise variance \\
		\verb!\ind!  & $\ind$ & Indicator function \\
		\verb!\batchconf!  & $\batchconf$ & Batch of $q$ candidate configurations \\
		\verb!\qEI!  & $\qEI$ & $q$-Expected Improvement acquisition \\
...
%		$\bonextobs$ & BO loop next observation \\
		...
		\verb!\sh!  & $\sh$ & Surrogate posterior standard deviation\\
		\verb!\cmin!  & $\cmin$ & Best (minimum) observed cost so far\\
		\verb!\PP!  & $\PP$ & Probability measure\\
		\bottomrule
\end{tabular}
\end{table}
