...
\begin{frame}{Local Search Considerations}
    \begin{itemize}
        ...
        \item Disadvantages
            \begin{itemize}
                \item Must define local neighborhoods (what is a small change to a hyperparameter), often needs discretization
...
\begin{frame}{Ablation Analysis \liturl{Fawcett and Hoos 2016}{https://www.cs.ubc.ca/labs/algorithms/Projects/Ablation/papers/FawcettHoos-joh2016-ablationAnalysis.pdf}}
    \begin{itemize}
        \item Use ``local search'' to determine important hyperparameters
        \item Start from initial configuration (e.g.\ default)
        \item Change configuration greedily, one hyperparameter at a time, until target
        ...
        \item Does not consider hyperparameter interactions and only evaluates
            local importance (specific to the particular pair of configurations)
    \end{itemize}
\end{frame}
...
\begin{enumerate}
    ...
    \item Ablation analysis can quantify hyperparameter importance with respect
        ...
        has found, but does not directly consider interactions
\end{enumerate}
\end{frame}
