...
\begin{itemize}
    \item If you already know a promising baseline, you should try these out first
    \item AutoML tools often need substantially more compute compared to a single model training
    \begin{itemize}
        \item If you would try something like $10$ different models, it could be worth considering AutoML already!
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item That is even more so if hyperparameters interact with each other
    \end{itemize}
...
\begin{itemize}
    \item If you deal with a well-known dataset (e.g., Imagenet), there are often very well-performing hyperparameter configurations for all kinds of thing
...
\begin{itemize}
    \item Some AutoML tools generate fairly complex ensemble/stacking models, which might be harder to interpret them simpler models with less accuracy.
\end{itemize}
