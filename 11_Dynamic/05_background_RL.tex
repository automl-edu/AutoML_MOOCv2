...
\begin{frame}{What Even Is a State?}
    ...
    For simplicity's sake, in the rest of the lecture, we assume that we can observe all information that is relevant for an agent to solve a task.\footnote{See chapter 8 of \liturl{Puterman and Chan, 2026}{https://github.com/martyput/MDP_book/blob/master/08\%20Chapter/Book___chapter08.pdf} for a detailed discussion of the partial observation setting.}
\end{frame}
%-----------------------------------------------------------------------
%----------------------------------------------------------------------
\begin{frame}{Actions}
    \begin{itemize}
        \item An action will induce some change in (the state of) the environment\pause
...
\begin{frame}{Rewards}
    \begin{itemize}
        \item Rewards provide crucial feedback on whether the outcome of an action was ``good'' or ``bad''\pause
...
\begin{frame}{Q-Learning}
    \begin{itemize}
        \item Most RL algorithms aim to estimate the State(-Action) Value, i.e., how valuable is it to be in a certain state\pause
        \item Value-based algorithms directly use this estimate to derive a policy\pause
        \item Q-learning \liturl{Watkins, 1989}{https://www.cs.rhul.ac.uk/~chrisw/new_thesis.pdf} is one of the most popular value-based RL algorithms:\pause
    \end{itemize}
    \begin{equation*}
        ...
        \underbrace{
            \Big(
                \overbrace{
                    r_{t} + \gamma \max_a\mathcal{Q}(s_{t+1},a)
                }^{\text{TD-target}}
                - \mathcal{Q}(s_t,a_t)
            \Big),
        }_{\text{TD-delta}}
...
\begin{frame}{Q-Learning Cont'd}
    ...
    \end{equation*}
    \begin{description}
        \item[TD-target] Discounted expected future rewards given the assumption that we will behave greedy wrt to $\mathcal{Q}$ in the future. The discounting factor $\gamma$ determines how strongly to weigh the influence of predicted future rewards.
...
\begin{frame}{The Policy of a Q-Learning Agent}
    ...
    \end{equation*}
    The policy then simply is $\policy(s) \in \argmax_{a}\mathcal{Q}(s,a)$
\end{frame}
%-----------------------------------------------------------------------
%----------------------------------------------------------------------
...
\begin{itemize}
    \item \textbf{Reinforcement learning agents generate the data they learn from.} This induces a stronger non-stationarity than supervised learning.
    \item \textbf{Q-learning estimates how good it is to traverse through a state.}
    \item \textbf{Policies maximize the future expected reward.}
\end{itemize}
