...
\begin{frame}{Dynamic Algorithm Configuration by RL}
    ...
    \begin{tikzpicture}[node distance=4cm]
        ...
        \draw[myarrow] (Agent.north) -- ($(Agent.north)+(0.0,+0.5)$) -- ($(Algo.north)+(0.0,+0.5)$) node [above=-.05,pos=0.5] { action $\policy(s_t) \mapsto a_t$} node [below,pos=0.5] {(set hyperparameter $\conf$)} -- (Algo.north);
    ...
    \begin{itemize}
        ...
        \item<2-> To be able to leverage the contextual MDP, we have to slightly modify the training and policy \liturl{Biedenkapp et al. 2020}{https://andrebiedenkapp.github.io/assets/pdf/paper/20-ecai-dac.pdf}
    \end{itemize}
\end{frame}
%-----------------------------------------------------------------------
%----------------------------------------------------------------------
\begin{frame}{General Policies}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Context-oblivious policies $\policy(s)$ only observe the state of the algorithm that is being~tuned
            \item Context-oblivious policies \ldots
            \begin{itemize}\pause
                \item generalize by training on a distribution of datasets.
                \item are robust and tolerate variation but can not perfectly adapt to a specific dataset.
                \item are simpler to realize than context-aware ones.
            \end{itemize}\pause
            \item Context-aware policies $\policy(s,\dataset)$ additionally observe meta-features of a given dataset
            \begin{itemize}\pause
...
Domains of change
    \begin{itemize}
        \item Directly setting the hyperparameter $\conf_{i,t} = \actionRL_t$
        \begin{itemize}\pause
        ...
        \begin{itemize}\pause
            \item Change the hyperparameter by a factor, e.g., $\conf_{i,t} = \actionRL_t\cdot\conf_{i,t-1}$
            \item Change the hyperparameter by a summand, e.g., $\conf_{i,t} = \actionRL_t+\conf_{i,t-1}$
...
\begin{frame}{Reward}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item BUT: Can lead to too aggressive hyperparameter changes early on to minimize cost quickly, but does not converge as well later on
...
\begin{frame}{Context-Awareness: Characterizing Datasets}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Might require a lot of training and a large history
        \end{itemize}\pause
        ...
        \item This simple approach often outperforms context-oblivious policies! See, e.g, \liturl{Benjamins et al. 2023}{https://openreview.net/forum?id=Y42xVBQusn} or \liturl{Prasanna et al. 2024}{https://openreview.net/forum?id=o8DrRuBsQb}\pause
