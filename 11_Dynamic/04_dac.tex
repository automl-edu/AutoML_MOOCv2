...
%-----------------------------------------------------------------------
%----------------------------------------------------------------------
\begin{frame}{Dynamic Algorithm Configuration (DAC)}
    \begin{itemize}
        \item Prior approaches achieve dynamic tuning but do not produce a policy\pause
        \item DAC Goal: A \alert{reusable \textit{policy}} that adapts hyperparameters\pause
    \end{itemize}
    Let
    \begin{description}
...
\begin{frame}{DAC as Contextual Markov Decision Process\\ \liturl{Biedenkapp et al. 2020}{https://andrebiedenkapp.github.io/assets/pdf/paper/20-ecai-dac.pdf}}
    \begin{description}
        ...
        \item[Transition] iteration of the algorithm for one or a few steps with the given hyperparameter settings\pause
...
\begin{frame}{DAC by Optimization (Functional Schedule)}
    \begin{itemize}
        ...
        \item You could use an evolutionary strategy (see, e.g.,\liturl{Chrbaszcz et al. 2018}{https://arxiv.org/abs/1802.08842} or \liturl{Fuks et al. 2019}{https://ml.informatik.uni-freiburg.de/wp-content/uploads/papers/19-IJCAI_PEL.pdf})
    \end{itemize}
\end{frame}
%-----------------------------------------------------------------------
%----------------------------------------------------------------------
...
\begin{itemize}
    \item \textbf{DAC generates reusable configuration policies.} Counter to approaches that simply adapt hyperparameters, the goal of DAC is to produce a policy.
    \item \textbf{DAC comes in a variety of flavors.} Depending on the choice of solution approach, the (learned) policies can be more or less expressive but accordingly might need more or less resources to learn them.
\end{itemize}
