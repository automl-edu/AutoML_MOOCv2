...
\begin{frame}[t]{Joint / $q$-Acquisition Functions}
  \begin{itemize}
    ...
    \item Example: $q$-Expected Improvement
        $$\acq_{\text{qEI}}(\mathbf{C}) = \E\!\left[\max_{k=1,\ldots,q} \max\!\left(\fmin - Y(\confI{k}),\,0\right)\right]$$
  ...
  \begin{enumerate}
    ...
    \item \textbf{Fantasize} the outcome at $\confI{1}$: replace the unknown $\cost(\confI{1})$ with\\
        \begin{itemize}
            ...
            \item Constant Liar: a fixed value (e.g. $\fmin$ or $\surro(\confI{1}) + c\sh(\confI{1})$)
