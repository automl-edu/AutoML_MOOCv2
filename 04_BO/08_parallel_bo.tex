...
\begin{frame}[t]{Why Parallel BO?}
  \begin{itemize}
    ...
    \item We can try to fully parallelize the underlying objective $\cost(\lambda)$,
...
\begin{frame}[t]{Kriging Believer / Fantasizing}
  ...
  \begin{itemize}
    \item Start with current surrogate; propose $\confI{1} \in \argmax \acq(\conf)$
...
\begin{frame}[t]{Asynchronous Decentralized BO}
  ...
  \begin{itemize}
    \item Synchronous batches stall the slowest worker\\
     $\Rightarrow$ idle compute under heterogenous runtimes. 
...
\begin{tikzpicture}[font=\scriptsize,
  ...
  \node[archive] (arch) at (0, 0) {\textbf{Shared archive}\\[-1pt]\scriptsize $(\confI{i}, \costI{i})$ + fantasies of pending};
  ...
  % Each pair of arrows: finish (up, green) and propose (down, blue), bent apart.
  \draw[finish ] (w1.north) to[bend left=15]
        node[midway, left=2pt, font=\tiny, color=mygreen!55!black] {$\cost$} (arch.south west);
  \draw[propose] (arch.south west) to[bend left=15]
        node[midway, right=2pt, font=\tiny, color=blue!60!black] {$\confI{\text{new}}$} (w1.north);

  \draw[finish ] (w2.north) to[bend left=10]
        node[midway, left=2pt, font=\tiny, color=mygreen!55!black] {$\cost$} (arch.south);
  \draw[propose] (arch.south) to[bend left=10]
        node[midway, right=2pt, font=\tiny, color=blue!60!black] {$\confI{\text{new}}$} (w2.north);

  \draw[finish ] (w3.north) to[bend right=15]
        node[midway, right=2pt, font=\tiny, color=mygreen!55!black] {$\cost$} (arch.south east);
  \draw[propose] (arch.south east) to[bend right=15]
        node[midway, left=2pt, font=\tiny, color=blue!60!black] {$\confI{\text{new}}$} (w3.north);

  % small legend
  \node[anchor=west, align=left, font=\tiny] at (4.2, -1.0)
    {\textcolor{mygreen!55!black}{$\rightarrow$ on finish: $\cost$}\\
     \textcolor{blue!60!black}{$\rightarrow$ refit, propose}};
\end{tikzpicture}
\end{center}
\end{frame}


\begin{frame}[t]{Asynchronous Distributed BO}
  \footnotesize
  \begin{itemize}
...
\begin{frame}[t]{Summary and Discussion}
  \begin{itemize}
    ...
    avoids idle workers under heterogenous runtimes
