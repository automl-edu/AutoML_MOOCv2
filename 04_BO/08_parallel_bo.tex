...
\begin{frame}[t]{$q$-LCB}
  \begin{itemize}
    \item Sample exploration weight per batch slot~\liturl{Hutter et al. 2012}{https://link.springer.com/chapter/10.1007/978-3-642-34413-8_5}: each $k = 1, \ldots, q$ draws $\tau_k \sim \mathrm{Exp}(1)$ and picks
        $$\confB{k} \in \argmin_{\conf} \;\surro(\conf) - \tau_k \sh(\conf)$$
    \item Different $\tau_k$ $\leadsto$ different mean / uncertainty trade-offs $\leadsto$ batch spreads automatically
    \item Embarrassingly parallel: no fantasy, no sequential refit
  \end{itemize}
\end{frame}
...
\begin{frame}[t]{Asynchronous Decentralized BO}
  \footnotesize
  \begin{itemize}
    \item Synchronous batches imply slowest eval slows the whole batch\\
...
\begin{tikzpicture}[font=\scriptsize,
  ...
  \node[archive] (arch) at (0, 0) {\textbf{Shared archive}\\[-1pt]\scriptsize real $(\confI{t}, \costI{t})$ + pending $\conf$ (no $\cost$ yet)};
...
\begin{frame}[t]{Asynchronous Decentralized BO}
  ...
  \begin{itemize}
    ...
    \item Archive holds real evals + a list of \textbf{pending} $\conf$ (busy on other workers, no $\cost$ yet)
    \item \textbf{On propose:} worker pulls the archive, locally \textbf{fantasizes} $\cost$ for each pending $\conf$
    (e.g., posterior mean / Kriging believer or a randomized variant), fits its surrogate on real + fantasy data, picks $\argmax \acq$
    % \item Fantasies are \textbf{local} to one propose call -- never persisted in the archive
    \item Fantasies inflate posterior certainty at pending $\conf$ $\leadsto$ workers' $\argmax \acq$ move elsewhere; batch stays diverse w.r.t.\ running work
  \end{itemize}
...
\begin{frame}[t]{Summary and Discussion}
  \begin{itemize}
    ...
    \item $q$EI, $qLCB$, Kriging believer / fantasizing, Thompson sampling as batch versions
