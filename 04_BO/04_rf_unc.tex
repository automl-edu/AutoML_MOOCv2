...
\begin{columns}[onlytextwidth,T]
  ...
  \begin{itemize}
    ...
    \item SMAC~\liturl{Hutter et al. 2011}{https://link.springer.com/chapter/10.1007/978-3-642-25566-3_40} uses BO with RF
  \end{itemize}
  \column{.4\textwidth}
  \begin{center}
    \includegraphics[width=\textwidth]{images/forest.png}
  \end{center}
\end{columns}
\end{frame}
...
\begin{frame}[t]{Jackknife-Based Variance Estimators}
  \begin{itemize}
    \item \textbf{Infinitesimal jackknife (IJ)}~\liturl{Wager et al. 2014}{https://jmlr.org/papers/v15/wager14a.html} gives a formally grounded variance estimate:
        $$\widehat{\sh}^2_{IJ}(\x) = \sum_{i=1}^N \Big(\operatorname{Cov}_b(\nu_{ib}, \hat{f}_b(\x))\Big)^2$$
        where $\nu_{ib}$ is how often observation $i$ appears in bootstrap sample $b$
...
\begin{itemize}
  \item Use RF for mixed / conditional $\pcs$, large $N$, cheaper $f$ with many iterations;
