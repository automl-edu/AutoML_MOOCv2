...
\begin{columns}[onlytextwidth,T]
  ...
  \begin{itemize}
    ...
    \item SMAC classical example, uses BO with RF
  \end{itemize}
  \column{.4\textwidth}
  \begin{center}
...
\begin{columns}[onlytextwidth,T]
  ...
  \column{.45\textwidth}
  \begin{center}
    \includegraphics[width=\textwidth]{images/rf_unc_naive.pdf}\\
...
\begin{columns}[onlytextwidth,T]
  \column{.5\textwidth}
  \begin{itemize}
    \item Standard RF: at each node, pick the \textbf{empirically best} split (max variance reduction)
    \item \textbf{Extra-trees}~\liturl{Geurts, Ernst, Wehenkel 2006}{https://link.springer.com/article/10.1007/s10994-006-6226-1}: pick the split point \textbf{uniformly at random} per candidate feature
    \item Trade-offs:
        \begin{itemize}
        ...
        \end{itemize}
    \item Often used in BO for smoother AF landscapes
  \end{itemize}
  \column{.5\textwidth}
  \begin{center}
    \includegraphics[width=\textwidth]{images/extra_trees.pdf}\\
...
\begin{columns}[onlytextwidth,T]
  ...
  \column{.4\textwidth}
  \begin{center}
    \includegraphics[width=\textwidth]{images/rf_unc_qrf.pdf}\\
...
\begin{itemize}
  ...
  GP for numeric low-to-medium dim with tighter budget
