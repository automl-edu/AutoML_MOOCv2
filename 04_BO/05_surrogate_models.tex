...
\begin{frame}{Gaussian Process}
    \small
    Posterior predictive distribution for test point $\xv \in \pcs$ under zero mean:
    $$Y(\xv) \mid \xv, \Dt \sim \normal\left(\surro(\xv), \sh^2(\xv)\right)$$
    with
    $$\surro(\xv) = k(\xv)^\top \bm{K}^{-1} \yv, \quad \sh^2(\xv) = k(\xv, \xv) - k(\xv)^\top \bm{K}^{-1} k(\xv)$$
    \begin{center}
        \includegraphics[width=.4\textwidth]{images/bayesian_loop_sm_normal.png}
    \end{center}
    \footnotesize Note: $\xv$ here denotes the test input. $k(\xv) := (k(\xv, \xvsi[1]), \ldots, k(\xv, \xvsi[t]))^\top$, $\yv := (\ysi[1], \ldots, \ysi[t])^\top$. Kernel/Gram matrix $\bm{K} := (k(\xvsi, \xvsi[j]))_{i,j}$ for $i, j \in \{1, \ldots, t\}$.
\end{frame}
...
\begin{frame}{Random Forest -- Mean and Variance}
    \small
    \begin{itemize}
        \item Let $\surro_b: \pcs \to \R$ be the mean prediction of tree $b$ (mean of all data points in the same node as $\xv$)
        \item Let $\sh_{b}^2: \pcs \to \R$ be the variance prediction (variance of all data points in the same node as $\xv$)
        \item Mean prediction of forest: $\surro: \pcs \to \R, \xv \mapsto \frac{1}{B} \sum_{b=1}^{B} \surro_{b}(\xv)$
        \item Variance prediction of forest:
        $$\xv \mapsto \left(\frac{1}{B} \sum_{b=1}^{B} \sh_{b}^2(\xv) + \surro_{b}(\xv)^{2}\right) - \surro(\xv)^{2}$$
