...
\subtitle{Noisy Bayesian Optimization}
...
\begin{frame}[t]{Noise-Aware Surrogate: Nugget GP}
  % \textbf{Issue:} an interpolating GP forces $\sh^2(\confI{t}) = 0$ at design points -- under noise the posterior should stay uncertain.
  \begin{columns}[onlytextwidth,T]
    \column{.55\textwidth}
    \begin{itemize}
      \item Add nugget $\noisevar \bm{I}$ to the kernel matrix:
        $$\surro(\conf) = \bm{k}_*^T (\bm{K} + \noisevar \bm{I})^{-1} \yv$$
        $$\sh^2(\conf) = k_{**} - \bm{k}_*^T (\bm{K} + \noisevar \bm{I})^{-1} \bm{k}_*$$
      \item Posterior \textbf{smooths} through the data instead of interpolating
      \item $\sh^2(\confI{t}) > 0$ even at design points
...
\begin{frame}[t]{Noise-Aware Acquisition: AEI and NEI}
  ...
  \begin{itemize}
    ...
    $\cmin^* = \min_t \surro(\confI{t}) + \beta\, \sh(\confI{t})$; risk-aversion $\beta \ge 0$ down-weights uncertain points
    ...
    \item \textbf{Noisy EI (NEI)}~\liturl{Letham et al. 2019}{https://projecteuclid.org/journals/bayesian-analysis/volume-14/issue-2/Constrained-Bayesian-Optimization-with-Noisy-Experiments/10.1214/18-BA1110.full}: MC generalization of reinterpolation -- per sample, draw plausible noiseless values at the design points from the GP posterior, refit an interpolating GP, compute noiseless EI; average across samples -- no plug-in
...
\begin{frame}[t]{Which Point to Return?}
  ...
  \begin{itemize}
    \item Empirical best $\argmin_t y_t$: noisy, vulnerable to lucky outliers
    \item \textbf{Model-based:} $\argmin_t \surro(\confI{t})$
    \item \textbf{Risk-averse:} $\argmin_t \surro(\confI{t}) + \beta\, \sh(\confI{t})$ (Huang's plug-in)
