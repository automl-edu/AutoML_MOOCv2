...
\begin{frame}[t]{Noise-Aware Acquisition: AEI and NEI}
  \small
  \textbf{Issue:} PI / EI use $\cmin = \min_i y_i$, but that is a noisy random variable $\leadsto$ lucky outliers dominate.
  \vspace{0.1cm}
...
\begin{frame}[t]{Which Point to Return?}
  Do \textbf{not} return $\argmin_i y_i$ -- the noisy empirical min: bad points get overrated by chance, good points overlooked.
  \vspace{0.1cm}
  \begin{itemize}
    ...
    \item \textbf{Posterior-mean argmin on the whole domain:} $\argmin_{\conf \in \pcs} \surro(\conf)$ -- may fall outside the evaluated archive, so only trust if the GP is well-calibrated
  \end{itemize}
  \vfill
  \begin{center}
    \includegraphics[width=.4\textwidth, height=2.5cm, keepaspectratio]{images/07_noisy_incumbent.pdf}
  \end{center}
\end{frame}
...
\begin{frame}[t]{Summary}
  \begin{enumerate}
    \item Noise breaks three things: interpolating surrogate, $\cmin$-based ACQFs, and empirical best-point identification
