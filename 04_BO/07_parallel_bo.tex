...
\begin{frame}[t]{Joint / $q$-Acquisition Functions}
  \small
  \begin{itemize}
    \item Extend acquisition to emph{joint} criterion over batch $\mathbf{C} = (\confI{1}, \ldots, \confI{q})$:
    ...
    the standard trick (BoTorch) is to reparameterize MC samples with fixed base draws $\xi$ 
    so $\acq_{\text{qEI}}$ becomes differentiable in $\mathbf{C}$ and can be optimized with Adam / L-BFGS. 
