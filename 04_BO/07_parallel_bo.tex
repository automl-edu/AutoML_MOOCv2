...
\begin{frame}[t]{Joint / $q$-Acquisition Functions}
  \small
  \begin{itemize}
    \item Extend acquisition to emph{joint} criterion over batch $\mathbf{B} = (\confI{1}, \ldots, \confI{q})$:
        % $$\acq_q(\mathbf{B}) = \E_{Y(\confI{1}), \ldots, Y(\confI{q})}\big[\, g(Y(\confI{1}), \ldots, Y(\confI{q}))\,\big]$$
    \item Example: $q$-Expected Improvement
        $$\acq_{\text{qEI}}(\mathbf{B}) = \E\!\left[\max_{k=1,\ldots,q} \max\!\left(\cmin - Y(\confI{k}),\,0\right)\right]$$
    ...
    $\leadsto$ $\acq_{\text{qEI}}(\mathbf{B})$ is smooth + differentiable, optimized with Adam / L-BFGS.
