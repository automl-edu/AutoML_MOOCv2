...
\begin{frame}{Optimization via Surrogate Modeling}
    ...
    \begin{itemize}
        \item We do not know the objective function $\cost: \pcs \to \R$
        \item But we can evaluate $\cost$ for a few different inputs $\xv \in \pcs$
        \item For now, we assume that those evaluations are noise-free
    \end{itemize}
    \textbf{Idea:} Use the data $\Dt = \{(\xvsi, \ysi)\}_{i = 1, \ldots, t}$ with $\ysi := \cost(\xvsi)$ to derive properties about the unknown function $\cost$.
    \vfill
    \begin{center}
        \includegraphics[width=.5\textwidth]{images/loop_1.png}
    \end{center}
\end{frame}
...
\begin{frame}{Basic Loop}
    ...
    \begin{enumerate}
        ...
        \item Optimize the surrogate model $\surro$ to obtain a new point $\xvsi[t+1] := \argmin_{\xv \in \pcs} \surro(\xv)$
        \item Evaluate $\xvsi[t+1]$ and update data $\Dt[t+1] = \Dt \cup \{(\xvsi[t+1], \cost(\xvsi[t+1]))\}$
    \end{enumerate}
\end{frame}
