...
\begin{itemize}
    ...
    \item Goal: draw general conclusions about relative performance that go beyond a single dataset
...
\begin{frame}{The No Free Lunch Perspective}
    \begin{itemize}
        \item No Free Lunch (NFL) theorem: no algorithm dominates all others across all possible tasks
        ...
        \item \textbf{Cherry-picking datasets} to favor one's own system is a common and serious threat to validity
        \item A credible benchmark should cover diverse tasks: varying size, feature types, class imbalance, and difficulty
    \end{itemize}
\end{frame}

\begin{frame}{Desiderata for a Fair Benchmark}
    \begin{itemize}
        \item Reproducibility: Fixed random seeds, public datasets, containerized environments
        \item Controlled resources: Same time budget, memory limit, and hardware for all systems
        \item Nested evaluation: Outer test folds held out from all systems, inner loop used for search
...
\begin{frame}{Exiting AutoML Benchmarks}
    \begin{itemize}
        \item OpenML-CC18: 72 curated classification datasets from OpenML, widely used as a default task suite
        \item AutoML Benchmark: Standardized framework for comparing AutoML systems with controlled resources and reproducible splits
        \item HPOBench / HPO-B: Focused benchmarks for hyperparameter optimization, providing surrogate evaluations or lookup tables for cheap repeated experiments
        \item YAHPO Gym: Surrogate-based benchmark suite covering a large space of HPO scenarios efficiently
        \item PD1: HPO for modern deep learning architectures (ResNets and Transformer)
    \end{itemize}
\end{frame}
